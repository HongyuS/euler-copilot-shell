...
-rwxr-xr-x. 1 root root      414 2025年 7月16日 oi-runtime.service
-rwxr-xr-x. 1 root root      409 2025年 7月16日 oi-rag.service
...
```bash
[root@localhost 5-resource]# oi-manager   #启动后直接输入 0 开启一键部署 等待部署完成，视网络情况好坏，大概在10~40分钟
一键部署菜单             
0) 自动部署
1) 手动部署
2) 重启服务
...
说明：0 是默认轻量部署，只部署 oi-runtime 后端服务；1 手动部署支持选择轻量部署还是全量部署，全量部署包含 Web 端和知识库。
...
#创建一一对应的agent应用
oi-manager --a create config.json
