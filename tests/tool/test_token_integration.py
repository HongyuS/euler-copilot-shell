...
class TestTokenIntegration:
    ...
    async def test_invalid_token_no_request(self, invalid_token_samples) -> None:
        """测试无效令牌不会发送 HTTP 请求"""
    ...
    async def test_valid_token_sends_request(self, valid_token_samples) -> None:
        """测试有效令牌会发送 HTTP 请求"""
        base_url = "http://localhost:8080"
    ...
    async def test_url_validation_priority(self, invalid_url: str) -> None:
        """测试 URL 格式验证优先于令牌验证"""
        # 使用有效令牌
        valid_token = "a1b2c3d4e5f6789012345678abcdef90"
